...
class Item(DocumentObject):
    ...
    def update_fields(
        ...
        side_mod_map = {mod.side: mod for mod in modification_buttons}

        for mod_side in mod_module.ModificationButtonSide._value2member_map_.values():
            button = side_mod_map.get(mod_side, None)
            set_update[f"isf.mod.{mod_side.value}"] = mod_module.ModificationButton.get_db_repr(button, False) if button is not None else None
